...
* **transient**: A Boolean indicating that the trait value is not persisted
  ...
  predefined traits is False. You can set it to True for traits whose values
  you know you do not want to persist. Do not set it to True on traits where
  it is set internally to False, as doing so is likely to create unintended
